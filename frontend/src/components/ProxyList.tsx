...
export default class ProxyIPList extends React.Component<Props, AppState> {
  ...
  async loadData(props: any) {
    ...
    const response = await axios.get(
      `${getBaseURL()}/api/v1/proxies?${queryString.stringify(params)}`
    )
  ...
  renderPagination(): JSX.Element {
    ...
    return <ul className="pagination">{pagination.map(e => e)}</ul>
  }
  ...
  private renderPageLink(pageNumber: number, label: string): JSX.Element {
    ...
    return (
      <li key={`page-nav-${pageNumber}`}>
        <Link to={`/?${queryString.stringify(parsed)}`}>{label}</Link>
      </li>
    )
  }
}
